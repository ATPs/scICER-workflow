...
if (length(args) < 1) {
  stop("Usage: Rscript codes/harmony_integration_scICER.R <input.qs> [batch_var] [celltype_var]")
}
...
sample_obj <- RunHarmony(
  ...
  assay.use = DefaultAssay(sample_obj),
  project.dim = FALSE,
  verbose = TRUE,
...
plots <- list(
  batch_after = DimPlot(sample_obj, reduction = "harmony_umap", group.by = batch_var, pt.size = 3, raster = TRUE) + base_theme,
  cluster_plot = DimPlot(sample_obj, reduction = "harmony_umap", group.by = "optimal_cluster", pt.size = 3, raster = TRUE) + base_theme,
