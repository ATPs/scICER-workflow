...
df_combined <- df %>%
  ...
  filter(method %in% c("Julia", "sclens") & (is.na(graph) | graph == "umap")) %>%
  transmute(sample, sample_type, source = case_when(method == "Julia" ~ "scICER (Julia UMAP)", method == "sclens" & graph == "umap" ~ "scICER (R UMAP)"), optimal = Optimal_Cluster_by_ECS, consistent_cluster)
...
for (stype in unique(df_combined$sample_type)) {
  ...
  df_sub <- df_sub %>% mutate(sample = factor(sample, levels = sample_levels), sample_num = as.numeric(sample), x_pos = ifelse(source == "scICER (Julia UMAP)", sample_num - 0.15, sample_num + 0.15))
  range_sub <- range_sub %>% mutate(sample = factor(sample, levels = sample_levels), sample_num = as.numeric(sample), x_pos = ifelse(source == "scICER (Julia UMAP)", sample_num - 0.15, sample_num + 0.15), end_adj = ifelse(start == end, start + 0.1, end), line_color = source)
  ...
  plot_list[[stype]] <- ggplot() +
    ...
    scale_color_manual(values = c("scICER (Julia UMAP)" = "#925E9FFF", "scICER (R UMAP)" = "#00A087FF"), name = "Method") +
