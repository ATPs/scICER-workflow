...
df_combined <- df %>%
  ...
  transmute(sample, sample_type, source = case_when(method == "Julia" ~ "scICER (Julia UMAP)", method == "umap_run_in_julia" ~ "scICE (reproduced)"), optimal = Optimal_Cluster_by_ECS, consistent_cluster)
...
for (stype in unique(df_combined$sample_type)) {
  ...
  df_sub <- df_sub %>% mutate(sample = factor(sample, levels = sample_levels), sample_num = as.numeric(sample), x_pos = ifelse(source == "scICE (reproduced)", sample_num - 0.15, sample_num + 0.15))
  range_sub <- range_sub %>% mutate(sample = factor(sample, levels = sample_levels), sample_num = as.numeric(sample), x_pos = ifelse(source == "scICE (reproduced)", sample_num - 0.15, sample_num + 0.15), end_adj = ifelse(start == end, start + 0.1, end), line_color = ifelse(source == "scICE (reproduced)", "scICE (reproduced)", "scICER (Julia UMAP)"))
  ...
  plot_list[[stype]] <- ggplot() +
    ...
    scale_color_manual(values = c("scICE (reproduced)" = "#4DBBD5FF", "scICER (Julia UMAP)" = "#925E9FFF"), name = "Method") +
